...
class ExcelService:
    """Service xử lý import/export Excel cho Derivative Mode - chunked export"""
    ...
    def read_excel_file(self, file_path: str) -> Tuple[bool, List[Tuple[str, str]], str]:
        """Đọc file Excel/CSV, luôn ưu tiên cột deriv_input"""
        try:
                    ...
                    for row_idx, row in enumerate(reader):
                        if row_idx == 0:
                            ...
                            if deriv_input_col_idx == -1:
                                return False, [], "❌ Không tìm thấy cột 'deriv_input'"
                        else:
                            if len(row) > deriv_input_col_idx and row[deriv_input_col_idx]:
                                deriv_input = row[deriv_input_col_idx].strip()
                                mode = "1"  # Default mode for derivative
