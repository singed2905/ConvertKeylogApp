...
class PolynomialEquationView:
    ...
    # ===================== UI =====================
    def _setup_ui(self):
        main = tk.Frame(self.window, bg="#F0F8FF");
    ...
    def _create_header(self, parent):
        ...
        tk.Label(bar, text="POLYNOMIAL EQUATION MODE v2.1", font=("Arial", 18, "bold"), bg="#1E3A8A", fg="white").pack(
            side="left", pady=20)
        status = "✅ Ready" if self.polynomial_service else "⚠️ Error"
        tk.Label(bar, text=f"Service: {status}", font=("Arial", 11), bg="#1E3A8A", fg="#B3D9FF").pack(side="right",
                                                                                                      padx=(0, 20),
                                                                                                      pady=(25, 15))

    def _create_control_panel(self, parent):
        frame = tk.LabelFrame(parent, text="⚙️ THIẾT LẬP PHƯƠNG TRÌNH", font=("Arial", 12, "bold"), bg="#FFFFFF",
                              fg="#1E3A8A", bd=2, relief="groove");
        frame.pack(fill="x", pady=10)
        r1 = tk.Frame(frame, bg="#FFFFFF");
        r1.pack(fill="x", padx=20, pady=12)
        tk.Label(r1, text="Bậc phương trình:", font=("Arial", 11, "bold"), bg="#FFFFFF", fg="#333", width=15).pack(
            side="left")
        cb_bac = ttk.Combobox(r1, textvariable=self.bac_phuong_trinh_var, values=["2", "3", "4"], state="readonly",
                              width=20, font=("Arial", 11));
        cb_bac.pack(side="left", padx=10);
        cb_bac.bind("<<ComboboxSelected>>", self._on_bac_changed)
        self.equation_form_label = tk.Label(r1, text="ax² + bx + c = 0", font=("Arial", 11, "italic"), bg="#FFFFFF",
                                            fg="#666");
        self.equation_form_label.pack(side="left", padx=20)
        r2 = tk.Frame(frame, bg="#FFFFFF");
        r2.pack(fill="x", padx=20, pady=(0, 12))
        tk.Label(r2, text="Phiên bản máy:", font=("Arial", 11, "bold"), bg="#FFFFFF", fg="#333", width=15).pack(
            side="left")
        cb_ver = ttk.Combobox(r2, textvariable=self.phien_ban_var, values=self.phien_ban_list, state="readonly",
                              width=20, font=("Arial", 11));
        cb_ver.pack(side="left", padx=10);
        cb_ver.bind("<<ComboboxSelected>>", self._on_phien_ban_changed)
        poly_cfg = self._get_polynomial_config();
        solver = poly_cfg.get('solver', {}).get('method', 'numpy') if poly_cfg else 'numpy'
        tk.Label(r2, text=f"Solver: {solver}", font=("Arial", 9), bg="#FFFFFF", fg="#666").pack(side="right", padx=20)

    def _create_quick_actions(self, parent):
        quick = tk.Frame(parent, bg="#F0F8FF");
        quick.pack(fill="x", pady=5)
        tk.Button(quick, text="📝 Tạo Template", command=self._create_template, bg="#1565C0", fg="white",
                  font=("Arial", 9, "bold")).pack(side="left", padx=2)
        tk.Button(quick, text="📁 Import Excel", command=self._on_import_excel, bg="#2196F3", fg="white",
                  font=("Arial", 9, "bold")).pack(side="left", padx=2)
        tk.Button(quick, text="🔥 Xử lý File Excel", command=self._on_process_excel, bg="#F44336", fg="white",
                  font=("Arial", 9, "bold")).pack(side="left", padx=2)

    def _create_guide_section(self, parent):
        guide = tk.LabelFrame(parent, text="💡 HƯỚNG DẪN NHẬP LIỆU", font=("Arial", 10, "bold"), bg="#E8F4FD",
                              fg="#1565C0", bd=1);
        guide.pack(fill="x", pady=5)
        text = ("• Nhập hệ số theo thứ tự từ cao đến thấp (a, b, c cho bậc 2)\\n"
                "• Hỗ trợ biểu thức: sqrt(5), sin(pi/2), 1/2, 2^3, log(10), \\\\frac{9}{4}\\n"
                "• Ô trống sẽ tự động điền số 0\\n"
                "• Phương trình dạng: ax^n + bx^(n-1) + ... + k = 0")
        tk.Label(guide, text=text, font=("Arial", 9), bg="#E8F4FD", fg="#333", justify="left", anchor="w").pack(
            side="left", padx=15, pady=10)

    def _create_input_section(self, parent):
        self.input_frame = tk.LabelFrame(parent, text="📝 NHẬP HỆ SỐ PHƯƠNG TRÌNH", font=("Arial", 12, "bold"),
                                         bg="#FFFFFF", fg="#1E3A8A", bd=2, relief="groove");
        self.input_frame.pack(fill="x", pady=10)

    def _create_roots_section(self, parent):
        self.roots_frame = tk.LabelFrame(parent, text="🎯 NGHIỆM PHƯƠNG TRÌNH", font=("Arial", 12, "bold"), bg="#FFFFFF",
                                         fg="#D35400", bd=2, relief="groove");
        self.roots_frame.pack(fill="x", pady=10)
        t = tk.Frame(self.roots_frame, bg="#FFFFFF");
        t.pack(fill="x", padx=15, pady=12)
        self.roots_text = tk.Text(t, width=80, height=8, font=("Courier New", 10), wrap=tk.WORD, bg="#FFF9E6",
                                  fg="#D35400");
        self.roots_text.pack(fill="x")
        self.roots_text.insert("1.0", "Chưa có nghiệm được tính")

    def _create_final_result_section(self, parent):
        self.final_frame = tk.LabelFrame(parent, text="📦 KẾT QUẢ TỔNG (CHO MÁY TÍNH)", font=("Arial", 12, "bold"),
                                         bg="#FFFFFF", fg="#2E7D32", bd=2, relief="groove");
        self.final_frame.pack(fill="x", pady=10)
        self.final_result_text = tk.Text(self.final_frame, width=80, height=3, font=("Courier New", 9, "bold"),
                                         wrap=tk.WORD, bg="#F1F8E9", fg="#2E7D32");
    ...
    def _create_control_buttons(self, parent):
        self.btn_copy_result = tk.Button(parent, text="📋 Copy Kết Quả", command=self._copy_result, bg="#9C27B0",
        ...
        # NEW: Split into 2 buttons - encode first, solve second
        tk.Button(self.frame_buttons_manual, text="🔑 Mã hóa Keylog", command=self._encode_only, bg="#13A05D",
                  fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_manual, text="🚀 Tính nghiệm & Mã hóa", command=self._process_polynomial,
                  bg="#4CAF50", fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_manual, text="💾 Xuất Excel", command=self._export_excel, bg="#FF9800", fg="white",
                  font=("Arial", 10, "bold")).pack(side="left", padx=5)

        # IMPORT MODE BUTTONS
        self.frame_buttons_import = tk.Frame(parent, bg="#F0F8FF");
        self.frame_buttons_import.pack(fill="x", pady=10)
        tk.Button(self.frame_buttons_import, text="📁 Import File Khác", command=self._on_import_excel, bg="#2196F3",
                  fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_import, text="🔥 Xử lý File Excel", command=self._on_process_excel, bg="#F44336",
                  fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_import, text="↩️ Quay lại", command=self._quit_import_mode, bg="#607D8B",
                  fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        self.frame_buttons_import.pack_forget()

    def _create_status_bar(self, parent):
        self.status_label = tk.Label(parent, text="🟢 Sẵn sàng nhập liệu phương trình bậc 2", font=("Arial", 10, "bold"),
                                     bg="#F0F8FF", fg="#2E7D32", relief="sunken", bd=1, anchor="w");
        self.status_label.pack(fill="x", pady=(10, 0))
        tk.Label(parent, text="Polynomial Mode v2.1 • Hỗ trợ giải phương trình bậc cao • Mã hóa tự động",
                 font=("Arial", 8), bg="#F0F8FF", fg="#666").pack(pady=5)
    ...
    def _on_phien_ban_changed(self, event=None):
        try:
            ...
            poly_cfg = self._get_polynomial_config();
            precision = poly_cfg.get('solver', {}).get('precision', 6) if poly_cfg else 6
    ...
    def _on_manual_input(self, event=None):
        ...
        if has_data and not self.manual_data_entered:
            ...
            self.manual_data_entered = False;
            self._update_button_visibility();
            self.status_label.config(text=f"🟢 Sẵn sàng nhập liệu phương trình bậc {self.bac_phuong_trinh_var.get()}",
                                     fg="#2E7D32")
    ...
    def _create_coefficient_inputs(self, bac):
        tk.Label(self.input_frame, text=f"Nhập {bac + 1} hệ số cho phương trình bậc {bac}:", font=("Arial", 10, "bold"),
                 bg="#FFFFFF", fg="#333").pack(anchor="w", padx=20, pady=10)
        container = tk.Frame(self.input_frame, bg="#FFFFFF");
        container.pack(fill="x", padx=20, pady=10)
        for label, var in self._get_coefficient_labels(bac):
            row = tk.Frame(container, bg="#FFFFFF");
            row.pack(fill="x", pady=5)
            tk.Label(row, text=label, font=("Arial", 10, "bold"), bg="#FFFFFF", fg="#1E3A8A", width=20,
                     anchor="w").pack(side="left")
            entry = tk.Entry(row, width=30, font=("Arial", 10), relief="groove", bd=2);
            entry.pack(side="left", padx=10)
            entry.bind('<KeyRelease>', self._on_manual_input)
            tk.Label(row, text=f"(hệ số {var})", font=("Arial", 9, "italic"), bg="#FFFFFF", fg="#666").pack(side="left",
                                                                                                            padx=10)
            self.coefficient_entries.append(entry)

    def _get_coefficient_labels(self, bac):
        cfg = {2: [("Hệ số a (x²):", "a"), ("Hệ số b (x):", "b"), ("Hệ số c (hằng số):", "c")],
    ...
    def _encode_only(self):
        """NEW: Encode coefficients to keylog without solving"""
        try:
    ...
    def _process_polynomial(self):
        try:
            ...
            self.window.update()
            success, status_msg, roots_display, final_keylog = self.polynomial_service.process_complete_workflow(
                coeff_inputs)
            if success:
                self.roots_text.config(state='normal');
                self.roots_text.delete("1.0", tk.END);
    ...
    def _copy_result(self):
        try:
            ...
            if text:
                self.window.clipboard_clear(); self.window.clipboard_append(text); messagebox.showinfo("Đã copy",
                                                                                                       f"Đã copy kết quả Polynomial vào clipboard:\\n\\n{text}")
            else:
    ...
    def _create_template(self):
        try:
            ...
            path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")],
                                                initialfile=f"polynomial_template_degree_{degree}.xlsx",
                                                title=f"Tạo Template cho Bậc {degree}")
            if not path: return
    ...
    def _on_import_excel(self):
        path = filedialog.askopenfilename(filetypes=[("Excel", "*.xlsx *.xls")],
                                          title="Chọn file Excel (sheet 'Input')")
        if not path: return
    ...
    def _on_process_excel(self):
        ...
        out_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel", "*.xlsx")],
                                                initialfile=default_name, title="Lưu file kết quả")
        ...
        try:
            ...
            processor.export_results(results_df, out_path,
                                     meta={"Source_File": os.path.basename(self.imported_file_path)})
    ...
    def _export_excel(self):
        try:
            if not self.has_result or not self.polynomial_service: messagebox.showwarning("Cảnh báo",
                                                                                          "Chưa có kết quả để xuất!\\n\\nVui lòng giải phương trình trước."); return
            output_path = filedialog.asksaveasfilename(title="Xuất kết quả Polynomial ra Excel",
                                                       defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")],
                                                       initialfile=f"polynomial_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
            ...
            info = self.polynomial_service.get_polynomial_info()
            df = pd.DataFrame({'Polynomial_Degree': [self.bac_phuong_trinh_var.get()],
                               'Calculator_Version': [self.phien_ban_var.get()],
                               'Polynomial_Form': [self.polynomial_service.get_polynomial_form_display()],
                               'Input_Coefficients': [' | '.join(input_data)], 'Encoded_Coefficients': [
                    ' | '.join(self.polynomial_service.get_last_encoded_coefficients())],
                               'Roots_Solution': [roots_text.replace('\\n', ' | ')], 'Final_Keylog': [final_result],
                               'Solver_Method': [info.get('solver_method', 'unknown')],
