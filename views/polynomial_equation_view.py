...
class PolynomialEquationView:
    """Full Polynomial Mode View: manual input, Excel batch, export, and keylog display"""
    ...
    def _create_quick_actions(self, parent):
        ...
        tk.Button(quick, text="🔥 Xử lý File Excel", command=lambda: PolynomialExcelUI.run_batch(self.window, lambda: self.bac_phuong_trinh_var.get(), lambda: self.phien_ban_var.get()), bg="#F44336", fg="white", font=("Arial", 9, "bold")).pack(side="left", padx=2)
    ...
    def _create_control_buttons(self, parent):
        ...
        # Manual action bar (always visible unless import mode)
        ...
        tk.Button(self.frame_buttons_import, text="📁 Import File Khác", command=self._import_excel, bg="#2196F3", fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_import, text="🔥 Xử lý File Excel", command=lambda: PolynomialExcelUI.run_batch(self.window, lambda: self.bac_phuong_trinh_var.get(), lambda: self.phien_ban_var.get()), bg="#F44336", fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
        tk.Button(self.frame_buttons_import, text="↩️ Quay lại", command=self._reset_all, bg="#607D8B", fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=5)
    ...
    def _reset_all(self):
        try:
            for e in self.coefficient_entries: e.delete(0, tk.END)
            ...
            self.final_result_text.config(state='normal'); self.final_result_text.delete("1.0", tk.END); self.final_result_text.insert("1.0", f"Polynomial Mode v2.1 - {'Service Ready' if self.polynomial_service else 'Service Failed'} | {'Config loaded' if self.config else 'Fallback config'}"); self.final_result_text.config(bg="#F1F8E9", font=("Courier New", 9), state='disabled')
            self.manual_data_entered=False; self.has_result=False; self.is_imported_mode=False; self._hide_copy_button(); self._update_button_visibility()
            if self.polynomial_service: self.polynomial_service.reset_state()
            self.status_label.config(text=f"🔄 Đã reset - Sẵn sàng nhập phương trình bậc {self.bac_phuong_trinh_var.get()}", fg="#2E7D32")
        except Exception as e:
