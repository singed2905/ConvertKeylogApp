...
class PolynomialEquationView:
    ...
    def _update_button_visibility(self):
        try:
            if self.is_imported_mode:
                ...
                self.frame_buttons_manual.pack_forget(); self.frame_buttons_import.pack_forget()
        except Exception:
            pass
    ...
    def _export_excel(self):
        try:
            ...
            export_data = {'Polynomial_Degree':[self.bac_phuong_trinh_var.get()], 'Calculator_Version':[self.phien_ban_var.get()], 'Polynomial_Form':[self.polynomial_service.get_polynomial_form_display()], 'Input_Coefficients':[' | '.join(input_data)], 'Encoded_Coefficients':[' | '.join(self.polynomial_service.get_last_encoded_coefficients())], 'Roots_Solution':[roots_text.replace('\n',' | ')], 'Final_Keylog':[final_result], 'Solver_Method':[polynomial_info.get('solver_method','unknown')], 'Real_Roots_Count':[len(self.polynomial_service.get_real_roots_only())], 'Export_Time':[datetime.now().strftime('%Y-%m-%d %H:%M:%S')]}
